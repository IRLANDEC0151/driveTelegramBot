import { choiceListRoutesInstance, createRouteInstance, startBotSceneInstance, startRaceInstance } from '../index.js'
    ...
    start() {
        try {
            ...
            this.scene.hears(/^(?!.*\/start).+$/, async ctx => {
                let route = (await Route.findOne({ name: ctx.message.text }))
                if (!route) {
                    ctx.scene.enter(choiceListRoutesInstance.name) 
                }else{

                    ctx.session.routeId = route.id
                    ctx.session.routeName = route.name   
